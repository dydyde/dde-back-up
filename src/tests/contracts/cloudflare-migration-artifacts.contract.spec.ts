...
describe('Cloudflare migration artifact contracts', () => {
  ...
  it('uses NanoFlow-Preview environment for preview builds while keeping public Sentry DSN optional but validated', () => {
    ...
    expect(workflow).toContain('NG_APP_SENTRY_DSN must be a valid public https Sentry browser DSN without a secret for one of: o4508391513718784.ingest.us.sentry.io, o4510686195023872.ingest.de.sentry.io');
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("allowedHosts.includes(url.hostname)");
    ...
    expect(workflow).not.toContain('actions/github-script@v7');
    expect(workflow).toContain('public_config_source="Repository secrets or variables"');
    expect(workflow).toContain('public_config_source="GitHub Environment');
    ...
    expect(workflow).toContain('short_sha="${GITHUB_SHA:0:7}"');
    expect(workflow).toContain('lookup_deploy_url_for_commit()');
    expect(workflow).toContain('querying deployments for commit $short_sha on branch $expected_branch');
    expect(workflow).toContain('grep -F "$short_sha"');
    expect(workflow).toContain('grep -F "$expected_branch"');
    expect(workflow).toContain('Wrangler finished without a deployment URL for commit $short_sha on branch $expected_branch');
    expect(workflow).not.toContain('deploy_url="https://${CLOUDFLARE_PAGES_PROJECT_NAME}.pages.dev"');
    expect(workflow).toContain('deployed_sha="$(printf \'%s\' "$version_json"');
    expect(workflow).toContain('[ "$deployed_sha" = "$GITHUB_SHA" ]');
    expect(workflow).toContain('Cloudflare Pages deployment did not become healthy for GITHUB_SHA=$GITHUB_SHA');
    expect(workflow).toContain('if: steps.deploy.outputs.deploy_url != \'\'');
    expect(workflow).toContain('origin="${{ steps.deploy.outputs.deploy_url }}"');
    expect(workflow).toContain('bash scripts/smoke/cloudflare-header-smoke.sh "${{ steps.deploy.outputs.deploy_url }}"');
  ...
  it('CSP allows Sentry regional ingest endpoints used by the production DSN', () => {
    ...
    expect(connectSrc).toContain('https://o4510686195023872.ingest.de.sentry.io');
