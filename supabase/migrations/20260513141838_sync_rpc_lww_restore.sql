-- =============================================================================
-- Sync RPC LWW restore after server-timestamp hardening
-- =============================================================================
--
-- Root cause:
-- The initial LWW restore (`20260509145500`) fixed merge/retry conflict storms by
-- removing strict CAS checks from sync upsert RPCs. Later non-task migrations
-- reintroduced `base_updated_at` equality checks for connection / blackbox /
-- project writes while switching those entities back to server `NOW()` stamps.
--
-- In production that leaves the conflict-resolution UI stuck in a loop:
-- merge writes mutate local timestamps, RPC interprets the stale base as a CAS
-- mismatch, and the app immediately re-enters `remote-newer` / conflict modal.
--
-- Fix:
-- Restore owner-only LWW upserts for task / connection / blackbox / project,
-- while keeping the newer server timestamp and tombstone protections.
-- =============================================================================
...
COMMENT ON FUNCTION public.sync_upsert_connection(JSONB) IS
  'Sync-protected connection upsert with idempotency, protocol fence, tombstone guards, ownership checks, and server-arrival LWW.';
