-- =============================================================================
-- Sync RPC server timestamps for non-task entities
-- =============================================================================
--
-- Root cause:
-- The LWW RPC rollout still accepted client-provided updated_at for
-- connection/project/blackbox inserts. Those tables only canonicalize updated_at
-- on UPDATE triggers, so a fresh INSERT could persist a skewed client timestamp
-- and poison later incremental-sync ordering.
--
-- Fix:
-- Always stamp updated_at on the server for non-task sync RPC writes.
-- Tasks continue to delegate to batch_upsert_tasks(), which already owns its
-- own server-side timestamp behavior.
-- =============================================================================
...
BEGIN
  ...
  SELECT c.project_id, p.owner_id, c.updated_at
  ...
  IF v_existing_updated IS NULL THEN
    ...
    END IF;
  ELSIF v_base_updated IS NULL OR v_base_updated <> v_existing_updated THEN
    INSERT INTO public.sync_operation_log (operation_id, user_id, entity_type, entity_id,
      status, reject_reason, protocol_version, deployment_epoch, deployment_target, client_git_sha, client_origin)
    VALUES (v_op_id, v_user, 'connection', v_conn_id, 'remote-newer',
...
BEGIN
  ...
  SELECT b.user_id, b.updated_at INTO v_existing_owner, v_existing_updated
